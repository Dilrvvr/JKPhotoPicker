...
- (UITableView *)tableView{
    if (!_tableView) {
        ...
        NSArray *tableViewCons2 = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-79-[tableView]-0-|" options:0 metrics:nil views:@{@"tableView" : tableView}];
...
- (void)initialization{
    ...
    navBgButton.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 64);
...
- (void)loadData{
    ...
    self.contentView.frame = CGRectMake(0, -[UIScreen mainScreen].bounds.size.height * 0.5 - 15, [UIScreen mainScreen].bounds.size.width, (rowHeight * self.albums.count + 64 > [UIScreen mainScreen].bounds.size.height * 0.5) ? [UIScreen mainScreen].bounds.size.height * 0.5 + 15 : rowHeight * self.albums.count + 64 + 15);
}
