...
- (UITableView *)tableView{
    if (!_tableView) {
        ...
        NSArray *tableViewCons2 = [NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"V:|-%.0f-[tableView]-0-|", JKNavBarHeight + 15] options:0 metrics:nil views:@{@"tableView" : tableView}];
...
- (void)initialization{
    ...
    navBgButton.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, JKNavBarHeight);
...
- (void)loadData{
    ...
    self.contentView.frame = CGRectMake(0, -[UIScreen mainScreen].bounds.size.height * 0.5 - 15, [UIScreen mainScreen].bounds.size.width, (rowHeight * self.albums.count + JKNavBarHeight > [UIScreen mainScreen].bounds.size.height * 0.5) ? [UIScreen mainScreen].bounds.size.height * 0.5 + 15 : rowHeight * self.albums.count + JKNavBarHeight + 15);
}
