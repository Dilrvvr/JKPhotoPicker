...
//        [self updateIconWithSourType:(UIImagePickerControllerSourceTypeCamera)];
        [JKPhotoPickerViewController showWithPresentVc:self maxSelectCount:7 seletedItems:nil shouldPreview:NO shouldSelectAll:YES dataType:JKPhotoPickerMediaDataTypeImageIncludeGif completeHandler:^(NSArray <JKPhotoItem *> *photoItems, NSArray<PHAsset *> *selectedAssetArray) {
    ...
    [alertVc addAction:[UIAlertAction actionWithTitle:@"从相册选择" style:(UIAlertActionStyleDefault) handler:^(UIAlertAction * _Nonnull action) {
        [JKPhotoPickerViewController showWithPresentVc:self maxSelectCount:0 seletedItems:nil shouldPreview:NO shouldSelectAll:YES dataType:JKPhotoPickerMediaDataTypeAll completeHandler:^(NSArray <JKPhotoItem *> *photoItems, NSArray<PHAsset *> *selectedAssetArray) {
            [self.imageView removeFromSuperview];
            self.imageView = nil;
            self.selectCompleteView.photoItems = photoItems;
            self.selectCompleteView.assets = selectedAssetArray;
