...
//        [self updateIconWithSourType:(UIImagePickerControllerSourceTypeCamera)];
        [JKPhotoPickerViewController showWithPresentVc:self maxSelectCount:7 seletedItems:self.selectCompleteView.photoItems dataType:JKPhotoPickerMediaDataTypeStaticImage isOpenCameraFirst:YES completeHandler:^(NSArray<JKPhotoItem *> *photoItems) {
    ...
    [alertVc addAction:[UIAlertAction actionWithTitle:@"从相册选择" style:(UIAlertActionStyleDefault) handler:^(UIAlertAction * _Nonnull action) {
        [JKPhotoPickerViewController showWithPresentVc:self maxSelectCount:7 seletedItems:self.selectCompleteView.photoItems dataType:JKPhotoPickerMediaDataTypeStaticImage isOpenCameraFirst:NO completeHandler:^(NSArray<JKPhotoItem *> *photoItems) {
            [self.imageView removeFromSuperview];
            self.imageView = nil;
            self.selectCompleteView.photoItems = photoItems;
        }];
